...
#  scale_colour_manual(values = line_colours) +
  ...
  theme_bw() +
theme(axis.text = element_text(size=12), # customize font size on x- and y- axis
        ...
        strip.text = element_text(size=9), # customize font size on the facets strip
        legend.background = element_rect(fill="transparent"), # customize colour of legend background 
        legend.position = c(0.08,0.85), # customize legend position 
...
```{r PmaxPlot, fig.cap = "Analysis of Maximum PSII Electron Transport Rate (Pmax) for *Prochlorococcus* MED4 (Clade HLI) and MIT9313 (Clade LLIV). Pmax,  derived from light response curve fits (XXXRefer to Figure), is plotted vs. measurement oxygen concentrations, with error bars representing the standard error of the estimate. Data is grouped by strain and growth light level, colours indicate different growth oxygen levels.", out.height="800pt", out.width="100%", echo = FALSE}
EstimatePmax <- ETRModelsParam |>
  ...
  #labs(title = ModType) +
  theme_bw() +
  theme(strip.background = element_rect(fill="white"), # customize colour of facet strip (default gray when using "theme_bw")
        strip.text = element_text(size=9), # customize font size on the facets strip
        legend.background = element_rect(fill="transparent"), # customize colour of legend background 
