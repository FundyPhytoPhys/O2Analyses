...
```{r LRC_fittedMIT9313, fig.cap = "Fitted LRC of PSII electron transport (e- PSII-1 s-1) vs measurement light (units) for *Prochlorococcus* strain MIT9313 (Clade ...) and after growth under different combinations of light level (30, 90, 180 units) and oxygen concentration (2, 25, 250 uM), derived from light response curve fits (XXXRefer to Figure). The curves are differentiated by varying oxygen concentrations (colors), with different panels representing different combinations of light levels and growth oxygen concentrations.", out.height="800pt", out.width="100%", echo = FALSE}}
ETRModels %>%
  filter(Strain == "MIT9313") %>%
  unnest(LRC_Augment) %>%
  ggplot(aes(x = ActPARCorr)) +
...
```{r PmaxPlot, fig.cap = "Analysis of Maximum PSII Electron Transport Rate (Pmax) for *Prochlorococcus* MED4 (Clade...) and MIT9313 (Clade...). Pmax,  derived from light response curve fits (XXXRefer to Figure), is plotted vs. measurement oxygen concentrations, with error bars representing the standard error of the estimate. Data is grouped by strain and growth light level, colours indicate different growth oxygen levels.", out.height="800pt", out.width="100%", echo = FALSE}
ETRModelsParam |>
  filter(ModelType == ModType) |>
  #filter(AIClow == 1) |>
