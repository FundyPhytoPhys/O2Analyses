...
#  scale_colour_manual(values = line_colours) +
  scale_x_continuous(expand = c(0,0)) +
  facet_nested(cols = vars(MeasuredO2_uM_lab, Strain, O2_uM), rows = vars(GrowthLight_lab, Par_ue), labeller = labeller(Par_ue_lab = label_parsed)) +
  labs(y = "e-" ~ PSII^{-1} ~s^{-1},
...
EstimatePmax <- ETRModelsParam |>
  ...
  mutate(Par_ue_lab = "Measurement Light~(µmol~photons~m^{-2}~s^{-1})",
         Strain_lab = ("Prochlorococcus Strain, Growth O2 (µM)")) %>% 
  ggplot() +
  ...
  facet_nested(rows = vars(Par_ue_lab, Par_ue), cols = vars(Strain_lab, Strain, O2_uM), labeller = labeller(Par_ue_lab = label_parsed)) + 
