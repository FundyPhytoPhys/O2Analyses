...
SolFitsMeta <- readRDS(file.path(DataIn, paste(paste(Project, Run, FileID, sep = "_"), ".Rds", sep = ""))) |>
  ...
  mutate(TauWT2 = (Tau1QA * Alp1QA) + (Tau2QA * Alp2QA),
         TauWT3 = (Tau1QA * Alp1QA) + (Tau2QA * Alp2QA) + (Tau3QA * Alp3QA))
...
SolFitsMeta %>%
  ...
  facet_grid(rows = vars(Dark1s), cols = vars(CultureID)) +
  theme_bw()
...
SolFitsMeta %>%
  filter(Dark1s == 0) |>
  ggplot() +
  geom_point(aes(x = LR_s, y = Tau1QA, colour = Ex_WL)) +
  #geom_point(aes(x = LR_s, y = Tau2QA, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(MeasureO2_uM), cols = vars(Strain, O2_uM, Par_ue)) +
  theme_bw()
...
SolFitsMeta %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = TauWT, colour = as.factor(Dark1s))) +
  facet_grid(rows = vars(MeasureO2_uM), cols = vars(Strain, O2_uM, Par_ue)) +
  theme_bw()
...
SolFitsMeta %>%
  ...
  geom_point(aes(x = LR_s, y = TauWT, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(MeasureO2_uM), cols = vars(Strain, O2_uM, Par_ue)) +
  theme_bw()
...
ETRModelsParam |>
  ...
  geom_errorbar(aes(x = Par_ue, y = estimate_a, ymin = (estimate_a - std.error_a), ymax = (estimate_a + std.error_a), colour = MeasureO2_uM)) +
...
ETRModelsParam |>
  ...
  geom_errorbar(aes(x = MeasureO2_uM, y = estimate_Pmax, ymin = (estimate_Pmax - std.error_Pmax), ymax = (estimate_Pmax + std.error_Pmax), colour = as.factor(Par_ue))) +
  facet_grid(rows = vars(O2_uM), cols = vars(Strain)) + 
  labs(title = ModType) +
  theme_bw()
...
ETRModelsParam |>
  ...
  geom_errorbar(aes(x = MeasureO2_uM, y = estimate_Ek, ymin = (estimate_Ek - std.error_Ek), ymax = (estimate_Ek + std.error_Ek), colour = as.factor(Par_ue))) +
  facet_grid(rows = vars(O2_uM), cols = vars(Strain)) + 
  labs(title = ModType) +
  theme_bw()
```
