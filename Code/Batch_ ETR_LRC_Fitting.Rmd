...
```{r Fitted,fig.cap = "Fitted LRC of PSII electron transport (e- PSII-1 s-1) vs measurement light (µmol photons m-2 s-1) for *Prochlorococcus* strains MIT9313 (Clade LLIV) and MED4 (and after growth under different combinations of light level (30, 90, 180 µmol photons m-2 s-1) and oxygen concentration (2, 25, 250 µM), derived from light response curve fits (XXXRefer to Figure). The curves are differentiated by varying oxygen concentrations (colors), with different panels representing different combinations of light levels and growth oxygen concentrations.", out.height="80pt", out.width="100%", echo = FALSE}
FittedLRC <- ETRModels %>%
  #filter(Strain == "MIT9313") %>%
  unnest(LRC_Augment) %>%
  mutate(GrowthLight_lab = "Growth Light (µmol photons m^-2 s^-1)",
         MeasuredO2_uM_lab = "~Prochlorococcus~ Strain; Growth O2 (µM)") %>% 
  ggplot(aes(x = ActPARCorr)) +
 # geom_point(aes(y = ETRqpOxbo, colour =  "ETRqpOxbo")) +
  geom_line(aes(y = `.fitted`, colour = MeasureO2_uM)) +
...
#  scale_colour_manual(values = line_colours) +
  scale_x_continuous(expand = c(0,0)) +
  facet_nested(cols = vars(MeasuredO2_uM_lab, Strain, O2_uM), rows = vars(GrowthLight_lab, Par_ue)) +
  labs(y = "Fitted LRC",
       x =  "PAR (µe)") +
  theme_bw() +
theme(legend.background = element_rect(fill="transparent"), # customize colour of legend background 
        legend.position = c(0.90,0.20), # customize legend position 
...
```{r PmaxPlot, fig.cap = "Analysis of Maximum PSII Electron Transport Rate (Pmax) for *Prochlorococcus* MED4 (Clade HLI) and MIT9313 (Clade LLIV). Pmax,  derived from light response curve fits, is plotted vs. measurement oxygen concentrations, with error bars representing the standard error of the estimate. Data is grouped by strain and growth light level, colours indicate different growth oxygen levels.", out.height="800pt", out.width="100%", echo = FALSE}
EstimatePmax <- ETRModelsParam |>
  filter(ModelType == ModType) |>
  #filter(AIClow == 1) |>
  filter(std.error_Pmax < 1 * estimate_Pmax) |>
  mutate(MeasuredO2_uM_lab = "Growth O2 (µM)",
         Strain_lab = "~Prochlorococcus~ Strain") %>% 
  ggplot() +
  geom_point(aes(x = MeasureO2_uM, y = estimate_Pmax, colour = as.factor(O2_uM))) +
  geom_line(aes(x = MeasureO2_uM, y = estimate_Pmax, colour = as.factor(O2_uM))) +
  ...
  facet_nested(rows = vars(MeasuredO2_uM_lab, O2_uM), cols = vars(Strain_lab, Strain)) + 
