...
Probably need an ANOVA of estimate_Pmax, and/or an ANOVA comparison of whether MeasureO2_uM affects the LRC model within a strain (and possibly within a growth light and a growth O2?)
https://stackoverflow.com/questions/45514426/run-multiple-model-and-save-model-comparison-results-in-dataframe-in-r
```{r ANOVA comparing models}
...
ETRanova <- ETRModels |>
  ...
  rename(O2_uM25 = `25`,
         ...
         O2_uM250 = `250`,
         GrowthO2_uM = O2_uM) |>
  #mutate(LRCanova = map2(O2_uM25, O2_uM2, ~anova(.x, .y))) 
  mutate(LRCanova = pmap(.l = list(O2_uM25, O2_uM2, O2_uM250), .f = anova()))
