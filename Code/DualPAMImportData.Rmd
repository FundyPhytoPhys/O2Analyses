...
Note: Added 'CultureID' column to googlesheet b/c ID numbers overlap with earlier samples
```{r load multiculticatalog, message = FALSE, warning = FALSE, echo=FALSE}
gs4_deauth()

#Increase guess_max to catch character codes in CultureID
...
DualPAMData |>
  ...
  filter(!(CultureID %in% c("JuNa1201", "JuNa1202", "JuNa1203", "JuNa1204", "JuNa1205"))) |>
  ggplot() +
  geom_point(aes(x = PAR, y = YI), colour = "blue") +
  geom_point(aes(x = PAR, y = YII), colour = "red") +
  facet_grid(cols = vars(MeasureO2_uM), rows = vars(CultureID)) +
...
DualPAMMeta <- left_join(x = DualPAMData, y = MetaData, join_by("CultureID" == "CultureID"))
...
DualPAMMeta |>
  ...
  geom_point(aes(x = PAR, y = YI), colour = "blue") +
  geom_point(aes(x = PAR, y = YII), colour = "red") +
  facet_grid(cols = vars(MeasureO2_uM), rows = vars(Strain, CultureID)) +
  theme_bw()

DualPAMMeta |>
  filter(MeasureO2_uM %in% c(2, 25, 250)) |>
  filter(!(CultureID %in% c("JuNa1201", "JuNa1202", "JuNa1203", "JuNa1204", "JuNa1205"))) |>
  filter(!(CultureID %in% c("JuNa1206", "JuNa1207", "JuNa1208", "JuNa1209", "JuNa1210"))) |>
  filter(CultureID != "JuNa1215") |>
  ggplot() +
  geom_point(aes(x = PAR, y = ETRI), colour = "blue") +
  geom_point(aes(x = PAR, y = ETRII), colour = "red") +
  facet_grid(cols = vars(MeasureO2_uM), rows = vars(Strain, Par_ue)) +
  theme_bw()
