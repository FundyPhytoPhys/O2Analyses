...
TestFile <- read_delim_plus(flnm = "../Data/RawData/DualPAM/Report/202311141453_PICO_JuNa1246_250.csv", delimiter = Delimiter, headerrows = DataHeaderRows, fileencode = FileEncode)

TestFile2 <- TestFile |>
    slice_tail(n = -(max(which(TestFile|> pull(Action) == "LC-Start"))-1)) 

library(rlang)

TestFile3 <- TestFile |>
    slice_tail(n = -(max(which({{"Action"}}) == "LC-Start"))-1)



|>
  
max(which(TestFile$Action == "LC-Start"))

LastLRCRow <- max(which(TestFile$Action == "LC-Start")) - 1
LastLRCRow

TestFile2 <- TestFile |>
  slice(max(LRCRow):nrow(TestFile))
...
```
Be Careful Here assumes all DualPAM LRC runs are 15 steps; that is NOT True.
Think of tags or something to mark 'different' runs, or figure out way to use text in file to ID requisite rows
...
read_delim_DualPAM2 <- function(flnm, delimiter, headerrows, fileencode){read_delim_plus(flnm, delimiter = Delimiter, headerrows = DataHeaderRows, fileencode = FileEncode)
  
TestFile2 <- TestFile |>
    slice_tail(n = -(max(which(TestFile$Action == "LC-Start")) - 1)) 
...
DualPAMMeta |>
  filter(MeasureO2_uM %in% c(2, 25, 250)) |>
  filter(!(SampleID %in% c("JuNa1201", "JuNa1202", "JuNa1203", "JuNa1204", "JuNa1205"))) |>
  #filter(!(SampleID %in% c("JuNa1206", "JuNa1207", "JuNa1208", "JuNa1209", "JuNa1210"))) |>
  filter(OptodeMeasure == 1) |>
  filter(Strain == "PCC9511") |>
  filter(SampleID != "JuNa1215") |>
  ggplot() +
  geom_point(aes(x = PAR, y = YI), colour = "blue") +
  geom_point(aes(x = PAR, y = YII), colour = "red") +
