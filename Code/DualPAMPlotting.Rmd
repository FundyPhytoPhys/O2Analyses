...
Y_PAR_PredictValues <- left_join(x = YFitsFilter |> unnest(YIaugmented), y = YFitsFilter |> unnest(YIIaugmented), by = join_by(Strain, O2_Category,Par_ue,  MeasureO2_uM, PAR), suffix = c(".YI", "YII")) |>
  select(c("Strain", "O2_Category", "Par_ue", "MeasureO2_uM", "PAR", "YI",".fitted.YI",".resid.YI","YII",".fittedYII",      ".residYII"))

Y_PAR_PredictValues2 <- left_join(x = Y_PAR_PredictValues, y = YFitsFilter |> unnest(qPaugmented), by = join_by("Strain", "O2_Category", "Par_ue", "MeasureO2_uM", "PAR"))
