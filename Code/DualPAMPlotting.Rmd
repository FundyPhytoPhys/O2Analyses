...
```{r YIIvsYI, fig.cap = "PSII electron transport (e- PSII-1 s-1) vs PSI electron transport (e- PSII-1 s-1) for *Prochlorococcus* strains PCC9511 (Clade ...) and MIT9313 (Clade LLV) after growth under different combinations of light level (30, 90, 180 µmol photons m-2 s-1) and oxygen concentration (2, 25, 250 µM). The blue and red points represent the experimental data.", out.height="800pt", out.width="100%", echo = FALSE}
YIIvsYI <- DualPAMMeta |>
  filter(MeasureO2_uM %in% c(2, 25, 250)) |>
  filter(!(SampleID %in% NoO2Runs)) |>
  filter(!SampleID %in% BadRuns) |>
  ...
  facet_grid(cols = vars(Strain, Par_ue, O2_Category), rows = vars(MeasureO2_uM)) +
  theme_bw()
...
```{r save spectra plots}
ggsave(filename = file.path(PlotsPathOut, "YIIvsYI.png"), plot = YIIvsYI)
```
