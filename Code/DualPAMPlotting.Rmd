...
ETRIvsETRII <- DualPAMMeta |>
  ...
  mutate(GrowthLight_lab = "Growth Light (µmol photons m^-2 s^-1)",
         MeasuredO2_uM_lab = "Strain; Growth O2 (µM)") %>% 
  #filter(O2_Category != "Low") |>
  #filter(Par_ue == 30) |>
  ggplot() +
  ...
  labs(y = "e-" ~ PSI^{-1} ~s^{-1},# "e- PSI^-1 s^-1",
       x =  "e-" ~ PSII^{-1} ~s^{-1},"e- PSII^-1 s^-1") +
  theme_bw()
ETRIvsETRII
