...
DualPAMSPData <- DualPAMSPFilterFiles[1:50] |>
...
DualPAMSPData <- DualPAMSPData |>
  ...
  separate_wider_delim(cols = Filename, delim = "/", names = c("Sample", "Flash")) |>
...
DualPAMSPData <- DualPAMSPData |>
  ...
  mutate(Flash = rank(MeasureTime, ties.method = "min")) |>
  ungroup()

head(DualPAMSPData)

unique(DualPAMSPData$Flash)

#Not quite what I seek...want to number Flashes  as sequential integers
...
DualPAMSPData |>
  ...
  facet_grid(rows = vars(MeasureO2_uM), cols = vars(Flash)) +
  theme_bw()


```
XXX To Here 27 Nov 2023 XXX
