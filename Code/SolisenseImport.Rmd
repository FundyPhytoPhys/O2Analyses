...
ExpDate <- "2023-05-27" #date of MultiCulti growth start
...
SolFitsTrim <- SolFits %>% 
  ...
  separate(Filename, into = c("Project", "RunDateTime", "CultureID", "Ex_WL", "O2_uM"), sep = "([\\/\\_])", remove = FALSE) %>%
  ...
  mutate(Ex_WL = as.factor(as.numeric(Ex_WL)),
         O2_uM = as.factor(as.numeric(O2_uM))) %>%
...
SolFitsTrim %>%
  ...
  filter(Tau2QA < 20000) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = FvFm, colour = as.factor(O2_uM))) +
...
SolFitsTrim %>%
  ...
  filter(Tau2QA < 20000) %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = Tau2QA, colour = as.factor(O2_uM), size = LR_s)) +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  theme_bw()
...
SolFitsTrim %>%
  #filter(Dark1s != 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Alp2QA, colour = as.factor(O2_uM), size = LR_s)) +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  theme_bw() 
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = ETRqpOxbo, colour = O2_uM)) +
  geom_line(aes(x = ActPARCorr, y = ETRqpOxbo, colour = O2_uM)) +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ETRqp, y = ETRqpOxbo, colour = O2_uM)) +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = C_WtTau, y = qpOxbo, colour =  O2_uM)) +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x =  JVPSII_aLHIIminmax, y = JVPSII_ETRqpOxbo, colour = O2_uM)) +
  #scale_colour_manual(values = Colours_nm) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
  #coord_fixed(ratio = 1) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
...
saveRDS(SolFitsTrim2, file.path(DataOut, paste(Project, ExpDate, "SolFitsTrim.Rds", sep = "_"), fsep = .Platform$file.sep))
```
