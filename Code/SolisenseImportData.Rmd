...
This .Rmd imports and tidys underlying data from the Solisense kinetic fluorometer software. It does not perform the underlying fits of the induction/relaxation profiles from FRRf protocols.
...
Many to many relation b/t SolDataMeta & ChlData
...
SolDataMeta |>
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = Fit/Chl)) +
...
SolDataMeta |>
  ...
  filter(LR_s < 70) |>
  ggplot() +
  geom_point(aes(x = log10(Curve_us), y = EM/Chl, colour = as.factor(ActPARCorr)), size = 0.2) +
  geom_line(aes(x = log10(Curve_us), y = Fit/Chl, colour = as.factor(ActPARCorr))) +
...
SolDataMeta |>
  ...
  filter(LR_s < 70) |>
  ggplot() +
  geom_point(aes(x = log10(Curve_us), y = EM/Chl, colour = as.factor(ActPARCorr))) +
  facet_grid(cols = vars(Strain, MeasureO2_uM), rows = vars(Par_ue, O2_uM)) +
  theme_bw()
```
